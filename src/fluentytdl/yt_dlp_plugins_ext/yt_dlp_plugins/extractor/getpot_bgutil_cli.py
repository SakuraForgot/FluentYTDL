...
class BgUtilCliPTP(BgUtilPTPBase):
    ...
    def _cli_path(self):
        ...
        deprecated_cli_path = self.ie._configuration_arg(
            ie_key="youtube", key="getpot_bgutil_script", default=[None]
        )[0]
        ...
        self.logger.debug(f"No CLI path found, defaulting to {default_path}")
        return default_path
    ...
    def _real_request_pot(
        ...
        self.logger.debug(
            f"Executing command to get POT via Rust executable: {' '.join(command_args)}"
        )
        ...
        try:
            ...
            raise PoTokenProviderError(
                f"_get_pot_via_cli failed: Unable to run executable (caused by {e!r})"
            ) from e
        ...
        try:
            ...
            raise PoTokenProviderError(
                f"Error parsing JSON response from _get_pot_via_cli (caused by {e!r})"
            ) from e
        if "poToken" not in cli_data_resp:
            raise PoTokenProviderError("The executable did not respond with a po_token")
