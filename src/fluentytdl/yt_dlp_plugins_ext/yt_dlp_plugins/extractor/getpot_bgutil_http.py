...
class BgUtilHTTPPTP(BgUtilPTPBase):
    ...
    def _base_url(self):
        ...
        deprecated_base_url = self.ie._configuration_arg(
            ie_key="youtube", key="getpot_bgutil_baseurl", default=[None]
        )[0]
    ...
    def _check_server_availability(self, ctx: PoTokenRequest):
        ...
        try:
            self.logger.trace(f"Checking server availability at {self._base_url}/ping")
            response = json.load(
            ...
            script_path_provided = (
                self.ie._configuration_arg(
                    ie_key="youtubepot-bgutilscript", key="script_path", default=[None]
                )[0]
                is not None
            )

            warning_base = (
                f"Error reaching GET {self._base_url}/ping (caused by {e.__class__.__name__}). "
            )
            if script_path_provided:  # server down is expected, log info
                ...
                self._warn_and_raise(
                    warning_base + f"Please make sure that the server is reachable at "
                    f"{self._base_url}."
                )
